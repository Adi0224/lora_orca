...
class wrapper2D(torch.nn.Module):
    def __init__(self, input_shape, output_shape, use_embedder=True, weight='base', train_epoch=0, activation=None, target_seq_len=None, drop_out=None, from_scratch=False, use_lora=False, lora_rank=4, lora_alpha=16, lora_dropout=0.1):
        super().__init__()
        ...
        if use_lora and not PEFT_AVAILABLE:
            raise RuntimeError("use_lora=True but peft is not installed. Install with: pip install peft")
        if use_lora and PEFT_AVAILABLE:
            print(f"\n{'='*60}")
            print(f"Adding LoRA to Swin Transformer")
            print(f"{'='*60}")

            # Target modules for Swin: query and value in attention
...
class wrapper1D(torch.nn.Module):
    def __init__(self, input_shape, output_shape, use_embedder=True, weight='roberta', train_epoch=0, activation=None, target_seq_len=512, drop_out=None, from_scratch=False, use_lora=False, lora_rank=4, lora_alpha=16, lora_dropout=0.1):
        super().__init__()
        ...
        if use_lora and PEFT_AVAILABLE:
            print(f"\n{'='*60}")
            print(f"Adding LoRA to {'RoBERTa' if weight[:7] == 'roberta' else 'BERT'}")
            print(f"{'='*60}")
...
def get_tgt_model(args, root, sample_shape, num_classes, loss, add_loss=False, use_determined=False, context=None, opid=0):
    ...
    tgt_model = wrapper_func(
        sample_shape, num_classes,
        weight=args.weight,
        train_epoch=args.embedder_epochs,
