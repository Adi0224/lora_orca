...
def load_state(use_determined, args, context, model, optimizer, scheduler, n_train, checkpoint_id=None, test=False, freq=1):
    ...
    if not test:
        ...
        rng_path = os.path.join(path, 'rng_state.ckpt')
        if os.path.isfile(rng_path):
            rng_state_dict = torch.load(rng_path, map_location='cpu')
            torch.set_rng_state(rng_state_dict['cpu_rng_state'])
            if rng_state_dict['gpu_rng_state'] is not None and torch.cuda.is_available():
