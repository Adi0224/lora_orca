#!/usr/bin/env python3
"""
ECG: FPT + LoRA Rank 2 (Baseline: No Alignment)
"""
import sys
import os
import json
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("  ECG: FPT + LoRA RANK=2 (RoBERTa, No Alignment)")
    print("="*70)
    print()
    ...
    with open('ORCA/src/configs/ecg_fpt_r2.yaml', 'r') as f:
    ...
    try:
        ...
        with open('results/ecg_fpt_r2_results.json', 'w') as f:
            json.dump(results, f, indent=2)

        print()
        print("="*70)
        print(f"✅ ECG FPT RANK=2 COMPLETE")
        print(f"  Test Accuracy: {test_acc*100:.2f}%")
        print(f"  Time: {elapsed:.1f}s")
        print(f"  Results saved to: results/ecg_fpt_r2_results.json")
        print("="*70)
